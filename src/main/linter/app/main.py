"""
FastAPI microservice — the Helm chart linting engine.
Accepts YAML file uploads, runs all checks, returns scored report.
"""
...
def extract_containers(chart_dict: dict) -> list:
    """Navigate the Kubernetes spec to find containers."""
    return (
        chart_dict
        .get("spec", {})
        .get("template", {})
...
async def lint_chart(file: UploadFile = File(...)):
    """Main endpoint — accepts YAML, returns full quality report."""
